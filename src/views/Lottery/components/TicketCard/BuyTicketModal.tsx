  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        availableSymbol="MIGO"
      />
      <div>
        <Tips>{t('1 Ticket = %lotteryPrice% MIGO', { lotteryPrice: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your MIGO cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %num% MIGO', { num: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
