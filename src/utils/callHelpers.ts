...
/**
 * Returns the total MIGO staked in the MIGO-BRISE LP
 * critical-search
 *
 */
export const getUserStakeInCakeBnbLp = async (account: string, block?: number) => {
  try {
    ...
    console.error(`MIGO-CORE LP error: ${error}`)
    return BIG_ZERO
  }
}

/**
 * Gets the MIGO staked in the main pool
 */
export const getUserStakeInCakePool = async (account: string, block?: number) => {
  try {
    ...
    console.error('Error getting stake in MIGO pool', error)
    return BIG_ZERO
  }
}
